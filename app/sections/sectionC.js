...
const SectionC = () => {
  ...
  useEffect(() => {
    ...
    onValue(sectionQuery, (snapshot) => {
      ...
      setLoading(false); coconutsList.sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  return (
    <ScrollView className="p-5 bg-white">
      <Text className="text-xl font-bold mb-5 text-center mt-10">Section C Coconuts</Text>
      {coconuts.length > 0 ? (
        coconuts.map((coconut) => (
          <View key={coconut.id} className="border p-3 rounded-lg mb-4">
            ...
            <TouchableOpacity onPress={() => router.push({ pathname: "../sections/updateSectionC", params: { id: coconut.id, coconut }  })} className="rounded-lg bg-red-300 h-[40px] w-[140px] ml-40 ">
